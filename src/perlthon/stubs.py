...
from .typed import _introspect_module, _python_safe_name
...
def _render_stub(module_name: str, functions: list[str]) -> str:
    ...
    used_names = {"available_functions"}
    lines = [
    ...
    for function_name in functions:
        if not function_name.isidentifier():
            continue
        stub_name = _python_safe_name(function_name)
        while stub_name in used_names:
            stub_name += "_"
        used_names.add(stub_name)
        lines.append("\n")
        if stub_name != function_name:
            lines.append(f"    # Perl: {function_name} -> Python: {stub_name}\n")
        lines.append(f"    def {stub_name}(self, *args: Any) -> Any:\n")
