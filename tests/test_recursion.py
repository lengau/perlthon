...
class TestPythonToPerlRecursion:
    ...
    def test_recursive_list_argument_raises_recursion_error(self):
        value: list[object] = []
        value.append(value)
    ...
    def test_recursive_dict_argument_raises_recursion_error(self):
...
class TestPerlToPythonRecursion:
    def test_recursive_array_result_raises_recursion_error(self):
        with pytest.raises(RecursionError, match=RECURSION_ERROR):
            perlthon.eval("do { my $a = []; push @$a, $a; $a }")

    def test_recursive_hash_result_raises_recursion_error(self):
        with pytest.raises(RecursionError, match=RECURSION_ERROR):
